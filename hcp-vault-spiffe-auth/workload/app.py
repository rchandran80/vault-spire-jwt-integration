...
def main():
    print(SEPARATOR)
    print("SPIRE + HCP Vault SPIFFE Auth Demo")
    print("Auth Method  : auth/spiffe/ (profile=static)")
    print("Bundle Source: static JWT key (manual refresh on SPIRE key rotation)")
    print("Vault Target : HCP Vault Dedicated")
    ...
    if not IN_DOCKER:
        ...
        result = subprocess.run(["docker", "ps", "--filter", "name=hcp-spire-agent", "--format", "{{.Names}}"],
                                capture_output=True, text=True)
        if "hcp-spire-agent" not in result.stdout:
            raise RuntimeError("hcp-spire-agent container is not running. Start it with: docker compose up -d spire-agent")
        print("  [OK] hcp-spire-agent container is running")
    else:
