...
def vault_login(jwt_token):
    print(f"\n{SEPARATOR}")
    print("STEP 2: Authenticate to Vault with JWT-SVID (SPIFFE auth method)")
    print(SEPARATOR)
    print(f"  Vault Address: {VAULT_ADDR}")
    print(f"  Auth Mount   : auth/spiffe/")
    print(f"  Role         : {VAULT_ROLE}")
    print(f"  Endpoint     : POST /v1/auth/spiffe/login")
    ...
    print(f"\n  Sending JWT-SVID via Authorization: Bearer header...")
    resp = requests.post(url, json={"role": VAULT_ROLE}, headers=headers)

    if not resp.ok:
        raise RuntimeError(f"Vault SPIFFE login failed {resp.status_code}: {resp.text}")
...
def read_secret(vault_token, path):
    print(f"\n{SEPARATOR}")
    print("STEP 3: Read Secret from Vault")
    print(SEPARATOR)
...
def main():
    print(SEPARATOR)
    print("SPIRE + Vault Enterprise SPIFFE Auth Demo")
    print("Auth Method  : auth/spiffe/ (profile=https_web_bundle)")
    print("Bundle Source: https://spire-server:8443 (SPIRE federation, native)")
    print("Vault Target : Vault Enterprise 2.0.0 (Docker spire-net)")
    print("Secret path  : secret/realpage/demo (KV-v2)")
    ...
    if not IN_DOCKER:
        ...
        result = subprocess.run(["docker", "ps", "--filter", "name=local-spire-agent", "--format", "{{.Names}}"],
                                capture_output=True, text=True)
        if "local-spire-agent" not in result.stdout:
            raise RuntimeError("local-spire-agent container is not running. Start it with: docker compose up -d spire-agent")
        print("  [OK] local-spire-agent container is running")
    else:
    ...
    print("  Vault         -> JWT-SVID validated via auth/spiffe/, Vault token issued")
